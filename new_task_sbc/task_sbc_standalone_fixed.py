...
def load_controlled(conn: psycopg.Connection, ranges_only: bool = False, quiet_missing_clients: bool = False) -> Tuple[set[str], list[Tuple[str, int, int]]]:
    ...
    with conn.cursor() as cur:
        ...
        if not ranges_only:
            try:
                sql = "SELECT ddd, number_local FROM controlled_number_ranges"
                log_sql(sql, None)
                cur.execute(sql)
                rows = cur.fetchall()
                logging.debug("controlled_number_ranges (ddd, number_local): %d linhas", len(rows))
                ...
                try:
                    sql = "SELECT full_number FROM controlled_number_ranges"
                    log_sql(sql, None)
                    cur.execute(sql)
                    rows = cur.fetchall()
                    logging.debug("controlled_number_ranges (full_number): %d linhas", len(rows))
                    for (full,) in rows:
                        f = norm(str(full))
                        if f:
                            nums.add(f)
                    ...
                    if not quiet_missing_clients:
                        logging.debug("Não foi possível ler controlled_number_ranges (%s)", e2)
